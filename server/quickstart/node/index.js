...
app.get('/api/auth', function (request, response, next) {
  Promise.resolve()
    .then(async function () {
      ...
      const AVAIABLE_BALANCE = authResponse.data.accounts.balances.available;
      const CURRENT_BALANCE = authResponse.data.accounts.balances.current;
      const NAME = authResponse.data.accounts.name;
      const SUB_TYPE = authResponse.data.accounts.subtype;
      response.json({
        available: AVAIABLE_BALANCE,
